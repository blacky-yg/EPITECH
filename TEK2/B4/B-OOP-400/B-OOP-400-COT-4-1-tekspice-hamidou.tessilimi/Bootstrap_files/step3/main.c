...
int main()
{
	...
	{
		list->add_front(list, test_contents[i], strlen(test_contents[i]));
	}
	printf("Elements in the list:\n");
	list->iter(list, &dump_string);

	if (list->mem(list, "meu", 3))
		printf("[meu] is in the list (OK)\n");
	else
	{
		printf("[meu] is not in the list, error!\n");
		return 1;
	}
	delete_list(list);
	 ...
	 */
	list = new_list_by_default();
	list->add_back(list, make_koala("zaz", 42, "nah, too lazy to do that."), sizeof(struct koala));
	list->add_back(list, make_koala("jack", 84, "That's a RUMOR!"), sizeof(struct koala));
	list->add_back(list, make_koala("thor", 168, "Proceed..."), sizeof(struct koala));
	printf("Koalas in the list:\n");
	list->iter(list, &dump_koala);
