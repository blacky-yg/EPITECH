...
static void test_populate_front()
{
	...
	{
		list->add_front(list, test_contents[i], test_lens[i]);
		assert(list->len = i + 1);
		assert(list->mem(list, test_contents[i], test_lens[i]) == 1);
	}
...
static void test_populate_back()
{
	...
	for (i = 0; i < test_qty; ++i)
	{
		list->add_back(list, test_contents[i], test_lens[i]);
		assert(list->len = i + 1);
		assert(list->mem(list, test_contents[i], test_lens[i]) == 1);
	}
...
static void test_mem()
{
	...
	assert(list->mem(list, "toto", 4) == 0);
	list->add_front(list, "toto", 4);
	assert(list->mem(list, "toto", 4) == 1);
}
